...
.cursor{--long-press-progress:0;position:absolute;left:6px;top:6px;width:28px;height:28px;border:3px solid #7cf7ff;border-radius:50%;box-sizing:border-box;box-shadow:0 0 12px #00d9ff;transition:transform 45ms linear,border-color 45ms linear,background 45ms linear,box-shadow 45ms linear}
.cursor::before{content:"";position:absolute;inset:-5px;border-radius:50%;background:conic-gradient(#ffcc66 calc(var(--long-press-progress)*1turn),transparent 0);-webkit-mask:radial-gradient(farthest-side,transparent calc(100% - 3px),#000 0);mask:radial-gradient(farthest-side,transparent calc(100% - 3px),#000 0);opacity:var(--long-press-progress);pointer-events:none}
...
.cursor.dragging{border-color:#ffcc66;background:#ffcc66;box-shadow:0 0 0 4px rgba(255,204,102,.3),0 0 16px #ff9900}
...
window.addEventListener('message',event=>{const data=event.data;if(!data||data.type!=='gesture-overlay')return;if(data.state)cursor.className='cursor '+data.state;if(Number.isFinite(data.longPressProgress)&&data.longPressProgress>=0&&data.longPressProgress<=1)cursor.style.setProperty('--long-press-progress',String(data.longPressProgress));if(data.pulse){pulse.className='pulse '+data.pulse;void pulse.offsetWidth;pulse.className='pulse '+data.pulse+' animate'}})
</script>`)} `);
